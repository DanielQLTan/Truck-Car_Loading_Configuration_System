...
function get_truck_info() {
	...
	ws.addEventListener("message", function(e) {
		var supp_truck = JSON.parse(e.data);
		truck_s.innerHTML = supp_truck.size;
		truck_p.src = supp_truck.image;
		for (let i = 0; i < supp_truck.angles.length; i++) {
			angle_list[i].src = supp_truck.angles[i]
		}
	});
};
